...
const Chatbot = () => {
    ...
    return (
        <div className="fixed bottom-20 right-8 flex flex-col items-end">
            <button onClick={alterChat} className="p-2 rounded-md bg-gray-900 hover:bg-blue-500 shadow-3xl">
                <img src="/img/logo.png" alt="Chatbot" className="w-12 h-12" />
            </button>
            {isOpen && (
                <div className="border opacity-80 p-2 bg-gray-900 shadow-lg rounded-md max-w-max m-2 fixed bottom-32 right-20">
                    <h2 className="text-3xl font-bold mb-4 text-center text-white">Chat</h2>
                    <div className="chatbot-messages max-h-64 overflow-y-auto">
                        {messages.map((message, index) => (
                            <div key={index} className={`message ${message.sender} text-white bg-blue-500 p-2 rounded-lg mb-2`}>
                                {message.text}
                            </div>
                        ))}
                    </div>
                    <div className="chatbot-input">
                        <input
                            ...
                            className="w-full px-4 py-2 bg-white rounded-lg"
                            value={inputValue}
                            onChange={alterInput}
                        />
                        <div className="flex justify-center mt-4">
                            <button onClick={sendMessage} className="bg-orange-500 text-white px-2 p-2 rounded-lg hover:bg-orange-600">Enviar</button>
                            <button onClick={exitChat} className="bg-orange-500 text-white px-2 p-2 rounded-lg hover:bg-orange-600">Sair do Chat</button>
                        </div>
                    </div>
                </div>
            )}
